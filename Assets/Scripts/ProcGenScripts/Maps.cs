...
public class Maps : MonoBehaviour
{
    ...
    public MeshMatData GetGeneratedData(Vector2 centre, Vector2 coord)
    {
        ...
        for (int i = 0; i < noiseSettings.SettingsVariations.Length; i++)
        {
            ...
            {
                firstLayerValue = NoiseGenerator.GenerateNoise(noiseSettings.SettingsVariations[0], seed, centre, noiseSettings.mapChunkSize);
            }

            SettingsVariation sv = noiseSettings.SettingsVariations[i];
            if(sv.enabled)
            {
                float[,] newMap = NoiseGenerator.GenerateNoise(sv, seed, centre, noiseSettings.mapChunkSize);
